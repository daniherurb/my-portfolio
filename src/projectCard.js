...
export default function ProjectCard({ project }) {
  return (
	<div className="flex flex-col justify-between h-[420px] sm:h-[520px] border-2 border-[var(--blue)] w-full p-5 rounded-md card gap-2">
	  <div className="flex flex-col sm:min-h-[130px]">
		...
		<p className="text-sm sm:text-md font-helios text-[var(--blue)] py-1"><b>tech: </b>{project.tech}</p>
		  {project.role && <p className="text-sm sm:text-md font-helios text-[var(--blue)] text-justify mb-1 line-clamp-4"><b>role: </b>{project.role}</p>}
	  </div>
